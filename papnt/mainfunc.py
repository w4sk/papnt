...
DEBUGMODE = False
converter = PDF2ChildrenConverter(load_config(Path(__file__).parent / "config.ini")["grobid"]["server"])
...
def add_records_from_local_pdfpath(
    ...
    for pdf_path in pdf_paths:
        ...
        if doi is None:
            ...
            continue
        try:
            prop = NotionPropMaker().from_doi(doi, propnames, registered_by, pdf_path.name) | {
                "info": {"checkbox": True}
            }
...
def update_unchecked_records_from_uploadedpdf(database: Database, propnames: dict):
    ...
    filter = {
        "and": [
            {"property": "info", "checkbox": {"equals": False}},
            {"property": propnames["pdf"], "files": {"is_not_empty": True}},
        ]
    }
    for record in database.fetch_records(filter).db_results:
        ...
        database.add_children(record["id"], children, blocktype="toggle", title="Text extracted by GROBID")
...
def make_bibfile_from_records(database: Database, target: str, propnames: dict, dir_save_bib: str):
    if dir_save_bib == "":
        raise RuntimeError('Edit "dir_save_bib" key in config.ini')

    propname_to_bibname = {val: key for key, val in propnames.items()}
    filter = {"property": propnames["output_target"], "multi_select": {"contains": target}}
    entries = [
        notionprop_to_entry(record["properties"], propname_to_bibname)
        for record in database.fetch_records(filter).db_results
    ]
...
if __name__ == "__main__":
    ...
    add_records_from_local_pdfpath(database, config["propnames"], "test/samplepdfs/sample1.pdf")
    update_unchecked_records_from_doi(database, config["propnames"])
    update_unchecked_records_from_uploadedpdf(database, config["propnames"])
    make_bibfile_from_records(database, "test", config["propnames"], config["misc"]["dir_save_bib"])
    make_abbrjson_from_bibpath(config["misc"]["dir_save_bib"] + "test.bib", config["abbr"])
