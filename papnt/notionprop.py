...
def to_notionprop(
    content: Optional[Any], mode: Literal["title", "select", "multi_select", "rich_text", "number", "date"]
):
...
class NotionPropMaker:
    ...
    def _fetch_info_from_arxiv(self, doi: str) -> dict:
        ...
        for author in paper.authors:
            authors.append({"given": " ".join(author.name.split(" ")[:-1]), "family": author.name.split(" ")[-1]})
    ...
    def _make_citekey(self, lastname, title, year):
        ...
        citekey = "".join([convert_lastname(lastname), make_shorttitle(title), str(year)])
    ...
    def _make_author_list(self, authors: List[dict]) -> List[str]:
        ...
        if len(authors_) > MAX_N_NOTION_MULTISELECT:
            ...
            self.notes.append("From the 100th to the second to last author" f': {"; ".join(extra_authors)}')
